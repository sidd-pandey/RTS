...
class Encoder(nn.Module):
    def __init__(self, opt):
        ...
        if self.opt.som_k >= 2:
            # second PointNet
            self.knnlayer = KNNModule(3+384, [64], activation=self.opt.activation, normalization=self.opt.normalization,
                                      momentum=opt.bn_momentum, bn_momentum_decay_step=opt.bn_momentum_decay_step, bn_momentum_decay=opt.bn_momentum_decay)

            self.cust_conv2d_1 = nn.Conv2d(1, 128, kernel_size=2, stride=2)
            self.cust_maxpl2d_1 = nn.MaxPool2d(2)
            self.cust_conv2d_2 = nn.Conv2d(128, 256, kernel_size=2, stride=2)
            self.cust_maxpl2d_2 = nn.MaxPool2d(2)
            # final PointNet
            self.final_pointnet = PointNet(3+256, (768, self.feature_num), activation=self.opt.activation, normalization=self.opt.normalization,
                                           momentum=opt.bn_momentum, bn_momentum_decay_step=opt.bn_momentum_decay_step, bn_momentum_decay=opt.bn_momentum_decay)
        else:
            # final PointNet
    ...
    def forward(self, x, sn, node, node_knn_I, is_train=False, epoch=None):
        ...
        if self.opt.som_k >= 2:
            ...
            self.conv2d_feature_1 = self.cust_conv2d_1(self.knn_feature_1.unsqueeze(1))
            self.maxpl_feature_1 = self.cust_maxpl2d_1(self.conv2d_feature_1)
            
            self.conv2d_feature_2 = self.cust_conv2d_2(self.conv2d_feature_1)
            self.maxpl_feature_2 = self.cust_maxpl2d_2(self.conv2d_feature_2)
            self.concat_feature = torch.cat((self.knn_center_1, self.maxpl_feature_2.view([8, 256, 64])), dim=1)
            # final pointnet --------------------------------------------------------------
            # self.final_pn_out = self.final_pointnet(torch.cat((self.knn_center_1, self.knn_feature_1), dim=1), epoch)  # Bx1024xM
            self.final_pn_out = self.final_pointnet(self.concat_feature, epoch)  # Bx1024xM
        else:
            # final pointnet --------------------------------------------------------------
